...
public class MultiCrafterAnalyzer {
    ...
    public static void analyzeRecipe(Map recipeMap, Seq<Recipe> to) {
        try {
            ...
            if (!recipe.isAnyEmpty())
                ...
                Log.warn("Recipe is empty, so skip it", recipe);
        } catch (Exception e) {
        ...
        if (ioEntry instanceof Map) {
            ...
            if (fluids != null) {
                if (fluids instanceof List) { // ["mod-id-item/1","mod-id-item2"]
                    ...
                    throw new RecipeAnalyzerException(
                            "Unsupported type of fluids at " + meta + " from <" + fluids + ">");
            }
            // power
            ...
            for (Object content : (List) ioEntry) {
                if (content instanceof String) {
                    ...
                    throw new RecipeAnalyzerException(
                            "Unsupported type of content at " + meta + " from <" + content + ">");
            }
    ...
    public static Item findItem(String id) {
        for (Item item : Vars.content.items())
            if (id.equals(item.name))
                return item;// prevent null pointer
        return null;
    }
    ...
    public static Liquid findFluid(String id) {
        for (Liquid fluid : Vars.content.liquids())
            if (id.equals(fluid.name))
                return fluid;// prevent null pointer
        return null;
    }
    ...
    public static Block findBlock(String id) {
        for (Block block : Vars.content.blocks())
            if (id.equals(block.name))
                return block; // prevent null pointer
        return null;
    }
    ...
    public static UnitType findUnit(String id) {
        for (UnitType unit : Vars.content.units())
            if (id.equals(unit.name))
                return unit; // prevent null pointer
        return null;
    }
    ...
    /**
     * Gets a field from a static class by name, throwing a descriptive exception if
     * not found.
     */
