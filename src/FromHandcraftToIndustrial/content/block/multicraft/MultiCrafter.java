package FromHandcraftToIndustrial.content.block.multicraft;
...
import FromHandcraftToIndustrial.content.block.multicraft.ui.*;
...
public class MultiCrafter extends Block {
     ...
     * Turn off this to ignore fluid output, for instance, the fluid is only
     * by-product.
     */
    ...
    public void init() {
        ...
        if (resolvedRecipes == null || resolvedRecipes.isEmpty())
            throw new ArcRuntimeException(MultiCrafterAnalyzer.genName(this)
                    + " has no recipe! It's perhaps because all recipes didn't find items or fluids they need. Check your `last_log.txt` to obtain more information.");
        if (selector == null)
    ...
    public class MultiCrafterBuild extends Building implements HeatBlock, HeatConsumer {
        /**
         * For {@linkplain HeatConsumer}, only enabled when the multicrafter requires
         * heat input
         */
            ...
            if (efficiency > 0 && (!hasPower || getCurPowerStore() >= cur.input.power)) {
                ...
                if (cur.isOutputFluid()) {
                    ...
                    for (LiquidStack output : cur.output.fluids) {
                        ...
                        handleLiquid(this, fluid,
                                Math.min(output.amount * increment, liquidCapacity - liquids.get(fluid)));
                    }
                }
                // particle fx
        ...
        public float getPowerProduction() {
            ...
            if (isOutputPower && cur.isOutputPower())
                return cur.output.power * efficiency;
            else
                return 0f;
        }
        ...
        public float efficiencyScale() {
            ...
            if (isConsumeHeat && cur.isConsumeHeat()) {
                ...
                return Math.min(Mathf.clamp(heat / heatRequirement) + over / heatRequirement * overheatScale,
                        maxEfficiency);
            } else
                return 1f;
        }
    ...
    public void buildStats(Table stat) {
        ...
        for (Recipe recipe : resolvedRecipes) {
            ...
            if (showNameTooltip)
                timeCell.tooltip(
                        Stat.productionTime.localized() + ": " + craftTime + " " + StatUnit.seconds.localized());
            // Output
    ...
    public void setBars() {
        ...
        if (hasPower)
            addBar("power", (MultiCrafterBuild b) -> new Bar(
                    b.getCurRecipe().isOutputPower()
                            ? Core.bundle.format("bar.poweroutput",
                                    Strings.fixed(b.getPowerProduction() * 60f * b.timeScale(), 1))
                            : "bar.power",
                    Pal.powerBar,
                    () -> b.efficiency));
        if (isConsumeHeat || isOutputHeat)
            addBar("heat", (MultiCrafterBuild b) -> new Bar(
                    b.getCurRecipe().isConsumeHeat() ? Core.bundle.format("bar.heatpercent", (int) (b.heat + 0.01f),
                            (int) (b.efficiencyScale() * 100 + 0.01f)) : "bar.heat",
                    Pal.lightOrange,
                    b::heatFrac));
    ...
    protected void setupConsumers() {
        ...
        if (isConsumePower)
            consume(new ConsumePowerDynamic(b -> ((MultiCrafterBuild) b).getCurRecipe().input.power));
    }
}
